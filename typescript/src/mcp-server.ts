...
import { getServerInfo } from "@/utils/manifest.js";
...
export class MCP {
	...
	get cache() {
		if (!this._cache) {
			...
			this._cache = new DurableObjectCache<State>(this.requestProperties.userHash, storage as unknown as DurableObjectStorage);
	...
	/*
	async init() {
         if (this.serverHasBeenInited) return;
        this.serverHasBeenInited = true;
