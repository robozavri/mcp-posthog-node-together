...
import { FunnelGetAllSchema } from "@/schema/tool-inputs";

const schema = FunnelGetAllSchema;

type Params = z.infer<typeof schema>;

export const getAllFunnelsHandler = async (context: Context, params: Params) => {
	...
	try {
		...
		const funnelInsights = result.results?.filter((insight: any) => {
		...
		if (funnelInsights.length === 0) {
			...
			const allInsightsUrl = `${getProjectBaseUrl(projectId)}/insights/`;
			...
			if (allResponse.ok) {
				...
				const insightTypes = allResult.results?.map((insight: any) => ({
					id: insight.id,
					name: insight.name,
				...
				const alternativeFunnels1 = allResult.results?.filter((insight: any) => {
					const isFunnel = (
						insight.filters?.insight === "funnels" ||
						insight.insight === "funnels" ||
						insight.query?.kind === "FunnelQuery" ||
				...
				const alternativeFunnels2 = allResult.results?.filter((insight: any) => {
					const isFunnel = (
						insight.name?.toLowerCase().includes("funnel") ||
						insight.filters?.display?.toLowerCase().includes("funnel")
					);
				...
				const alternativeFunnels3 = allResult.results?.filter((insight: any) => {
					// Check if it has funnel-like structure
					const isFunnel = (
						insight.filters?.events?.length > 1 ||
				...
				const alternativeFunnels4 = allResult.results?.filter((insight: any) => {
				...
				const alternativeFunnels5 = allResult.results?.filter((insight: any) => {
					// Look for insights that have step-by-step structure
					const events = insight.filters?.events || insight.query?.series || [];
					const hasSequentialSteps = events.length > 1 && events.every((event: any, index: number) => {
				...
				console.log("Funnel IDs found:", allPossibleFunnels.map((f: any) => f.id));
				...
				if (allPossibleFunnels.length > 0) {
					const formattedFunnels = allPossibleFunnels.map((funnel: any) => {
				...
				const allInsightsFormatted = allResult.results?.map((insight: any) => ({
					id: insight.id,
					short_id: insight.short_id,
			...
			const noFilterUrl = `${getProjectBaseUrl(projectId)}/insights/`;
			...
			if (noFilterResponse.ok) {
				...
				const noFilterFunnels = noFilterResult.results?.filter((insight: any) => {
					return (
						insight.filters?.insight === "funnels" ||
						insight.insight === "funnels" ||
						insight.query?.kind === "FunnelQuery" ||
				...
				if (noFilterFunnels.length > 0) {
					const formattedFunnels = noFilterFunnels.map((funnel: any) => ({
						id: funnel.id,
						short_id: funnel.short_id,
						name: funnel.name || funnel.filters?.display || "Unnamed Funnel",
						description: funnel.description || "No description",
			...
			const diagnosticInfo = {
				...
				allInsightsTypes: allResponse.ok ? (await allResponse.json()).results?.map((insight: any) => insight.insight) || [] : [],
				apiUrl: url,
		...
		const formattedFunnels = funnelInsights.map((funnel: any) => ({
			id: funnel.id,
			short_id: funnel.short_id,
			name: funnel.name || funnel.filters?.display || "Unnamed Funnel",
			description: funnel.description || "No description",
