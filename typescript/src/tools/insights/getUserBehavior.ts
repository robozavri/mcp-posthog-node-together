...
import { UserBehaviorSchema } from "@/schema/tool-inputs";
...
const schema = UserBehaviorSchema;

type Params = z.infer<typeof schema>;

export const getUserBehaviorHandler = async (context: Context, params: Params) => {
	...
	const totalSessions = formattedResults.reduce((sum: number, item: { total_sessions: number }) => sum + item.total_sessions, 0);
	const totalPageViews = formattedResults.reduce((sum: number, item: { total_page_views: number }) => sum + item.total_page_views, 0);
	const totalBounceSessions = formattedResults.reduce((sum: number, item: { bounce_rate_percent: number; total_sessions: number }) => 
		sum + (item.bounce_rate_percent * item.total_sessions / 100), 0);
	...
	const averageSessionDuration = formattedResults.length > 0 ? 
		formattedResults.reduce((sum: number, item: { average_session_duration_seconds: number }) => sum + item.average_session_duration_seconds, 0) / formattedResults.length : 0;
	...
	const mostEngagedUser = formattedResults.length > 0 
		? formattedResults.reduce((best: any, current: any) => 
			current.average_session_duration_seconds > best.average_session_duration_seconds ? current : best
		) : null;

	const leastEngagedUser = formattedResults.length > 0 
		? formattedResults.reduce((worst: any, current: any) => 
			current.average_session_duration_seconds < worst.average_session_duration_seconds ? current : worst
		) : null;

	const mostActiveUser = formattedResults.length > 0 
		? formattedResults.reduce((best: any, current: any) => 
			current.total_sessions > best.total_sessions ? current : best
		) : null;
...
const tool = (): Tool<typeof schema> => ({
	...
	`,
	schema,
	handler: getUserBehaviorHandler,
});
