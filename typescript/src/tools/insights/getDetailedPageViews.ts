...
export const getDetailedPageViewsHandler = async (context: Context, params: Params) => {
	...
	const totalBounceSessions = formattedResults.reduce((sum: number, item: { bounce_sessions: number }) => sum + item.bounce_sessions, 0);
	...
	const bestEngagementPage = formattedResults.length > 0 
		? formattedResults.reduce((best: any, current: any) => 
			current.avg_views_per_visitor > best.avg_views_per_visitor ? current : best
		) : null;

	const worstBounceRatePage = formattedResults.length > 0 
		? formattedResults.reduce((worst: any, current: any) => 
			current.bounce_rate_percent > worst.bounce_rate_percent ? current : worst
		) : null;
...
const tool = (): Tool<typeof schema> => ({
	...
	`,
	schema,
	handler: getDetailedPageViewsHandler,
});
