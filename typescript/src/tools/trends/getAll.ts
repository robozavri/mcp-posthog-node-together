...
import { TrendsGetAllSchema } from "@/schema/tool-inputs";

const schema = TrendsGetAllSchema;

type Params = z.infer<typeof schema>;

export const getAllTrendsHandler = async (context: Context, params: Params) => {
	...
	try {
		...
		const trendInsights = result.results?.filter((insight: any) => {
		...
		if (trendInsights.length === 0) {
			...
			if (allResponse.ok) {
				...
				const alternativeTrends1 = allResult.results?.filter((insight: any) => {
					const isTrend = (
						insight.filters?.insight === "trends" ||
						insight.insight === "trends" ||
						insight.query?.kind === "TrendsQuery" ||
				...
				const alternativeTrends2 = allResult.results?.filter((insight: any) => {
					const isTrend = (
						insight.name?.toLowerCase().includes("trend") ||
						insight.filters?.display?.toLowerCase().includes("trend")
					);
				...
				const alternativeTrends3 = allResult.results?.filter((insight: any) => {
				...
				const alternativeTrends4 = allResult.results?.filter((insight: any) => {
				...
				console.log("Trend IDs found:", allPossibleTrends.map((t: any) => t.id));
				...
				if (allPossibleTrends.length > 0) {
					const formattedTrends = allPossibleTrends.map((trend: any) => {
		...
		const formattedTrends = trendInsights.map((trend: any) => ({
			id: trend.id,
			short_id: trend.short_id,
			name: trend.name || trend.filters?.display || "Unnamed Trend",
			description: trend.description || "No description",
