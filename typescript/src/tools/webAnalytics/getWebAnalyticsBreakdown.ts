...
import { WebAnalyticsBreakdownSchema } from "@/schema/tool-inputs";
import { getProjectBaseUrl } from "@/lib/utils/api";
import { handleToolError } from "@/lib/utils/handleToolError";
import type { Context, Tool } from "../types";

type Params = z.infer<typeof WebAnalyticsBreakdownSchema>;

const schema = WebAnalyticsBreakdownSchema;

export const getWebAnalyticsBreakdownHandler = async (context: Context, params: Params) => {
	try {
		...
		return handleToolError(error, "Failed to get web analytics breakdown");
	}
};
...
async function getWebAnalyticsBreakdown(context: Context, projectId: string, startDate: string, endDate: string, groupBy: string) {
...
const tool = (): Tool<typeof schema> => ({
	...
	schema,
	handler: getWebAnalyticsBreakdownHandler,
});
