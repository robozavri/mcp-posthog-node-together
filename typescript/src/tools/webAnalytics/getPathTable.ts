...
import { PathTableSchema } from "../../schema/tool-inputs";
import { handleToolError } from "../../lib/utils/handleToolError";
import { getProjectBaseUrl } from "../../lib/utils/api";

type Params = z.infer<typeof PathTableSchema>;
...
async function getPathTableData(context: Context, projectId: string, startDate: string, endDate: string, limit: number) {
    ...
    return currentResults.map((row: any[]) => {
        ...
        const previousData = previousMap.get(path) || { visitors: 0, views: 0 };
        ...
        return {
            ...
            bounce_rate: {
                current: 0, // Temporarily set to 0 due to HogQL limitations
                previous: 0,
                change: "No change"
            }
        };
...
export const getPathTableHandler = async (context: Context, params: Params) => {
    try {
        console.log('Path table handler started with params:', params);
        ...
        const pathData = await getPathTableData(context, projectId, startDate, endDate, limit);
        console.log('Path table result:', pathData);
        ...
        console.error('Error in path table handler:', error);
        return handleToolError(error, "Failed to get path table data");
    }
};

export default function getPathTable() {
    return {
        name: "get-path-table",
        description: "Get page path statistics with visitors, views, and bounce rate in a table format",
        schema: PathTableSchema,
        handler: getPathTableHandler
    };
} 