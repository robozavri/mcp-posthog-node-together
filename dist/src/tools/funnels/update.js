...
exports.updateFunnelHandler = void 0;
const api_1 = require("@/lib/utils/api");
const tool_inputs_1 = require("@/schema/tool-inputs");
const schema = tool_inputs_1.FunnelUpdateSchema;
const updateFunnelHandler = async (context, params) => {
    const { projectId, insightId, data } = params;
    // Build the updated funnel insight configuration
    const insightConfig = {};
    if (data.name !== undefined)
        insightConfig.name = data.name;
    if (data.description !== undefined)
        insightConfig.description = data.description;
    if (data.steps || data.date_from || data.date_to || data.step_order ||
        ...
        if (data.date_from !== undefined)
            insightConfig.filters.date_from = data.date_from;
        if (data.date_to !== undefined)
            insightConfig.filters.date_to = data.date_to;
        if (data.step_order !== undefined)
            insightConfig.filters.step_order = data.step_order;
        if (data.conversion_rate_calculation !== undefined)
            insightConfig.filters.conversion_rate_calculation = data.conversion_rate_calculation;
        if (data.breakdown_by !== undefined)
            insightConfig.filters.breakdown_by = data.breakdown_by;
        if (data.breakdown_attribution_type !== undefined)
            insightConfig.filters.breakdown_attribution_type = data.breakdown_attribution_type;
        if (data.breakdown_attribution_step !== undefined)
            insightConfig.filters.breakdown_attribution_step = data.breakdown_attribution_step;
        ...
        if (data.global_filters !== undefined)
            insightConfig.filters.global_filters = data.global_filters;
        if (data.graph_type !== undefined)
            insightConfig.filters.graph_type = data.graph_type;
        if (data.first_occurrence !== undefined)
            insightConfig.filters.first_occurrence = data.first_occurrence;
    }
...
exports.updateFunnelHandler = updateFunnelHandler;
const tool = () => ({
    name: "update-funnel",
    description: `
    ...
    `,
    schema,
    handler: exports.updateFunnelHandler,
});
