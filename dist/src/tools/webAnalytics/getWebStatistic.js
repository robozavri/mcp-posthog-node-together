...
exports.getStatisticHandler = void 0;
const tool_inputs_1 = require("@/schema/tool-inputs");
const api_1 = require("@/lib/utils/api");
const handleToolError_1 = require("@/lib/utils/handleToolError");
const schema = tool_inputs_1.WebAnalyticsStatisticSchema;
const getStatisticHandler = async (context, params) => {
    try {
        ...
        return (0, handleToolError_1.handleToolError)(error, "Failed to get web analytics statistics");
    }
};
exports.getStatisticHandler = getStatisticHandler;
function calculateDateRanges(period, customDays) {
    const now = new Date();
...
async function getWebAnalyticsStats(context, projectId, startDate, endDate) {
...
function calculateMetricsWithComparison(current, previous) {
    const calculatePercentageChange = (current, previous) => {
        if (previous === 0) {
            return current > 0 ? 100 : 0;
        }
    ...
    const calculateBounceRate = (bounceSessions, totalSessions) => {
        if (totalSessions === 0)
            return 0;
...
const tool = () => ({
    ...
    schema,
    handler: exports.getStatisticHandler,
});
